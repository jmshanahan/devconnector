...
router.post('/', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  profileFields.company = 'my company';
  profileFields.website = 'www.company.au';
  profileFields.location = 'England';
  profileFields.bio = 'my bio';
  profileFields.status = 'Employed';
  profileFields.skills = ['php', 'javascript', 'hurling'];
  Profile.findOne({ user: req.user.id }).then(profile => {
    if (profile) {
      // Update
      // Profile.findOneAndUpdate({ user: req.user.id }, { $set: profileFields }, { new: true })
      Profile.findOneAndUpdate({ handle: 'BradTrav' }, { $set: { website: 'www.joe.ie' } }, { new: true })
